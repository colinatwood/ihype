...
| `src/app/api/artist-media/route.ts` | Every track upload (not just free-use ones) now runs AI metadata vetting via `vetFreeUseSample()`, **plus real audio-content vetting** via `vetTrackAudioContent()` (`src/lib/media-vetting.ts` — transcribes vocals/lyrics via Workers AI Whisper and checks for a known-commercial-song match; instrumental tracks pass through untouched, this is a lyrics check not a full acoustic fingerprint). Either check flagging marks the track not-cleared — fail-open, still publishes on a flag, but raises a `ContentReport` (`targetType: 'track'`) into the existing `/admin/moderation` queue. |
