...
| `src/proxy.ts` | HTTPS enforcement; www→apex redirect; auth protection for `/listen` (`WORKBENCH_PATH` in `src/lib/auth-redirects.ts`), `/dashboard`, `/admin`; CSP/security headers (renamed from `middleware.ts` per Next.js 16's proxy convention) |
