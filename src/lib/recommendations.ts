...
type Signals = { taste: number | null; geo: number | null; social: number; momentum: number; collab: number | null; comparable: number | null };

function finalScore(signals: Signals): number {
  let weightedSum = 0;
  let totalWeight = 0;
...
function buildReason(
  ...
  switch (top) {
    case 'taste': {
      // Find an overlapping genre that maps to an artist the viewer hyped.
