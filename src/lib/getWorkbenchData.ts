...
export async function getWorkbenchData(userId: string): Promise<WorkbenchData> {
  try {
    ...
    const responseData: WorkbenchData = {
      ...
      pageEditorsByType: user.profiles.length > 0
        ? Object.fromEntries(user.profiles.map((p) => [
            p.type,
            {
              profileId: p.id,
              slug: p.slug,
              ...
              topFiveContent: p.topFiveContent ?? '',
              mediaContent: p.mediaContent ?? '',
              nowPlaying: p.nowPlaying ?? '',
              links: p.links ?? '',
              merchUrl: p.merchUrl ?? '',
              merchContent: p.merchContent ?? '',
              tourContent: p.tourContent ?? '',
              requestContent: p.requestContent ?? '',
              upcomingContent: p.upcomingContent ?? '',
              previousShowHighlights: p.previousShowHighlights ?? '',
              ...
              stayRecommendations: p.stayRecommendations ?? '',
              heroImage: p.heroImage ?? '',
              avatarImage: p.avatarImage ?? '',
              logoImage: p.logoImage ?? '',
              galleryImage: p.galleryImage ?? '',
              featureVideoUrl: p.featureVideoUrl ?? '',
              themePreset: p.themePreset ?? '',
              themeAccentTone: p.themeAccentTone ?? '',
              themeBackdropTone: p.themeBackdropTone ?? '',
              fanShareEnabled: p.fanShareEnabled ?? false,
              songs: [],
              upcomingShows: [],
              previousShows: [],
            } satisfies import('@/types/workbench').WbPageEditor,
          ]))
        : undefined,
    };
