  ...
  return (
    <div style={{ borderRadius: 18, border: '1px solid var(--line)', background: 'var(--bg-surface)', overflow: 'hidden', marginBottom: 12, animation: `fadeIn .3s ${idx * .07}s both` }}>
      ...
      <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <button aria-label={hyped ? 'Remove hype' : 'Hype this show'} onClick={() => setHyped(h => !h)} style={{ width: 44, height: 44, borderRadius: '50%', border: `1px solid ${hyped ? 'var(--accent)' : 'var(--line)'}`, background: hyped ? 'rgba(255,80,41,.12)' : 'transparent', color: hyped ? 'var(--accent)' : 'var(--ink-3)', cursor: 'pointer', fontSize: 16, display: 'grid', placeItems: 'center' }}>{hyped ? '🔥' : '☆'}</button>
        <button aria-label="Share" style={{ width: 44, height: 44, borderRadius: '50%', border: '1px solid var(--line)', background: 'transparent', color: 'var(--ink-3)', cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
