...
export async function POST(request: NextRequest) {
  ...
  try {
    const profile = await withDbRetry(() => db.profile.findUnique({ where: { id: profileId }, select: { id: true } }));
...
export async function GET(request: NextRequest) {
  ...
  try {
    const count = await withDbRetry(() => db.follow.count({ where: { followeeProfileId: profileId } }));
    const session = await auth();
    let following = false;
    if (session?.user?.id) {
